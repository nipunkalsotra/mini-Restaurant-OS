...
class Restaurant(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "restaurant_phone IS NULL OR length(restaurant_phone) BETWEEN 10 AND 15",
            name="rs_ph_ch"
        ),
...
class Customer(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "customer_phone IS NULL OR length(customer_phone) BETWEEN 10 AND 15",
            name="len_csph"
        ),
    )
...
class Order(Base):
    ...
    __table_args__ = (
        CheckConstraint("table_number IS NULL OR table_number >= 0", name="tb_num_neg"),
